...
public class ImageUtils {
     ...
     * @param srcImageFile 源图像文件地址
     * @param result 缩放后的图像地址
     * @param limit_height 缩放后的高度限制（不能超过该高度）
     * @param limit_width 缩放后的宽度限制（不能超过该宽度）
     *
     */
    public final static void scale3 (String srcImageFile, String result,
                                     int limit_height,int limit_width){
        try {
            BufferedImage src = ImageIO.read(new File(srcImageFile)); // 读入文件
            double width_scale = src.getWidth()/(double)limit_width; // 得到源图宽度与宽度限制的比例
            double height_scale = src.getHeight()/(double)limit_height; // 得到源图长度与长度限制的比例
            double scale = width_scale > height_scale ? width_scale : height_scale;
            // 缩放
            int width = (int)Math.floor(src.getWidth() / scale);
            int height = (int)Math.floor(src.getHeight() / scale);
    ...
    public final static void scale2(String srcImageFile, String result, int height, int width, boolean bb) {
        try {
            ...
            if((bi.getHeight() < height && bi.getWidth() < width))
                throw new InvalidRequestException("图片格式不正确");
            // 计算比例
            if ((bi.getHeight() > height) || (bi.getWidth() > width)) {
    ...
    public final static void convert(String srcImageFile, String formatName, String destImageFile) throws IOException {
        ...
        BufferedImage newBufferedImage = new BufferedImage(src.getWidth(),
